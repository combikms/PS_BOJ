...
public class Main {
    ...
    public static void Quick_Sort_String(String[] arr, int left, int right) {
        if (left < right) {
            int pivotIndex = Partition_String(arr, left, right);
            Quick_Sort_String(arr, left, pivotIndex - 1);
            Quick_Sort_String(arr, pivotIndex + 1, right);
        }
    }
    
    private static int Partition_String(String[] arr, int left, int right) {
        ...
        for (int j = left; j < right; j++) {
            if (arr[j].length() < pivot.length() || (arr[j].length() == pivot.length() && arr[j].compareTo(pivot) < 0)) { 
                i++;
